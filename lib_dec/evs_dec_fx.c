...
)
{
    ...
    IF( sub(st_fx->codec_mode,MODE1) == 0 )
    {
        ...
        IF ( sub(st_fx->extl_fx,SWB_TBE) == 0 || sub(st_fx->extl_fx,FB_TBE) == 0
             || (sub(coder_type,AUDIO) != 0 && sub(coder_type,INACTIVE) != 0 && L_sub(st_fx->core_brate_fx,SID_2k40) > 0 && sub(st_fx->core_fx,ACELP_CORE) == 0
                 ...
                 && !( sub( st_fx->nelp_mode_dec_fx, 1) == 0 && sub( st_fx->bfi_fx, 1) == 0 ) ) )
        ...
        IF( sub(output_frame,L_FRAME32k) >= 0 )
        {
            ...
            test();
            if( L_sub(st_fx->core_brate_fx, SID_2k40) <= 0 && st_fx->bws_cnt_fx == 0 )
            {
                hb_synth_fx_exp = 3;
                move16();
            }
        ...
        IF (sub(st_fx->core_fx,ACELP_CORE) == 0
            && !st_fx->bfi_fx
            && st_fx->prev_bfi_fx
            && L_sub(st_fx->last_total_brate_fx, HQ_48k) >= 0
            && sub(st_fx->last_codec_mode, MODE2) == 0
        ...
        IF ( sub(st_fx->extl_fx,-1) != 0 || (st_fx->bws_cnt_fx > 0 && sub(st_fx->core_fx,ACELP_CORE) == 0) )
        {
            ...
            test();
            test();
            IF( ( st_fx->ppp_mode_dec_fx || (sub(st_fx->nelp_mode_dec_fx,1) == 0 && sub( st_fx->bfi_fx, 1 ) == 0 ) )  && sub(st_fx->L_frame_fx, st_fx->last_L_frame_fx) == 0 && (st_fx->bws_cnt_fx > 1 || st_fx->last_extl_fx != -1) )
    ...
    ELSE /* MODE2 PART */
    {
        ...
        IF( st_fx->igf )
        {
            ...
            test();
            test();
            if( L_sub(st_fx->output_Fs_fx,8000) == 0 || ( L_sub(st_fx->output_Fs_fx,16000) == 0 && sub(st_fx->L_frame_fx,L_FRAME16k) == 0 ) )
            {
                st_fx->extl_fx = -1;
                move16();
            }
            ...
            IF( sub(st_fx->m_frame_type, ACTIVE_FRAME) == 0 )
            {
                ...
                ELSE
                {
                    IF( sub(st_fx->last_core_fx,ACELP_CORE) == 0 )
                    {
                        ...
                        test();
                        test();
                        IF( ( sub(st_fx->bwidth_fx, SWB) == 0 || sub(st_fx->bwidth_fx, FB) == 0 ) &&
                        (( sub(st_fx->last_extl_fx, SWB_TBE) == 0 || sub(st_fx->last_extl_fx, FB_TBE) == 0) && sub(st_fx->last_codec_mode, MODE2) == 0 ) )
                        {
...

        {
            ...
            test();
            test();
            IF( st_fx->flag_cna == 0 && sub(st_fx->L_frame_fx,L_FRAME16k) == 0 && sub(st_fx->last_flag_cna,1) == 0
                && ( (st_fx->last_core_fx == ACELP_CORE && sub(st_fx->last_coder_type_fx,AUDIO) != 0) || sub(st_fx->last_core_fx,TCX_20_CORE) == 0 || sub(st_fx->last_core_fx,AMR_WB_CORE) == 0 ) )
            {
            ...
            ELSE
            {
                ...
                ELSE IF( sub(st_fx->L_frame_fx,L_FRAME16k) == 0 && sub(st_fx->last_flag_cna,1) == 0 && ( (sub(st_fx->last_core_fx,ACELP_CORE) == 0 && sub(st_fx->last_coder_type_fx,AUDIO) != 0 ) || sub(st_fx->last_core_fx,TCX_20_CORE) == 0 || sub(st_fx->last_core_fx,AMR_WB_CORE) == 0) )
                {
                    FOR( i=0; i < st_fx->L_frame_fx/2; i++ )
                    {
        ...
        IF( st_fx->igf != 0 && sub( st_fx->m_frame_type, ACTIVE_FRAME ) == 0 )
        {
            ...
            test();
            IF( (( ( st_fx->bfi_fx == 0 || st_fx->last_core_fx == ACELP_CORE ) && st_fx->core_fx == ACELP_CORE ) ||
                 ( ( sub(st_fx->last_core_fx, ACELP_CORE) == 0 ) && (sub(st_fx->bwidth_fx, NB) != 0 && sub(st_fx->last_codec_mode, MODE2) == 0) ))
                && (L_sub( st_fx->output_Fs_fx, 8000 ) > 0) )
            {
