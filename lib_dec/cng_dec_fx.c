...
)
{
    ...
    *allow_cn_step = 0;
    ...
    IF( L_sub(core_brate,SID_1k75) == 0 || L_sub(core_brate, SID_2k40) == 0 )
    {
      ...
      IF( L_sub(st_fx->last_core_brate_fx, SID_2k40) <= 0 || sub(st_fx->prev_bfi_fx, 1) == 0 )
      {
      ...
      IF(sub(L_enr_index, tmp1) > 0 && st_fx->old_enr_index_fx >= 0 )  /* Likely bit error and not startup */
      {
