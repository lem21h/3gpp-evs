...
)
{
    ...
    ELSE
    {
        ...
        ELSE    /* !is_transient */
        {
            ...
            test();
            IF( sub(nBits, 9 + NBITS_GLOB_GAIN_BWE_HR) >= 0 && sum16_fx( nq, Nsv) > 0 )
            {
                ind1 = (Word16) get_next_indice_fx( st_fx, NBITS_GLOB_GAIN_BWE_HR );
            ...
            test();
            IF( sub(st_fx->extl_fx,SWB_BWE_HIGHRATE) == 0 || sub(temp,shl(18022,sub(15-13,temp2))) < 0 )          /* 2.2 in Q13 == 18022 */
            {
