...
)
{
    ...
    IF( st_fx->bws_cnt_fx == 0 )
    {
        ...
        IF( !st_fx->bfi_fx )
        {
            ...
            {
                IF(sub(st_fx->last_extl_fx, WB_TBE) != 0)
                {
                    st_fx->GainFrame_prevfrm_fx = 0;
                ...
                IF( sub(st_fx->rf_frame_type,RF_NELP) == 0 )
                {
                    ...
                    IF( sub(st_fx->core_fx,ACELP_CORE) == 0 && sub(st_fx->last_core_fx,ACELP_CORE) == 0
                        ...
                        && L_sub(GainFrame,st_fx->GainFrame_prevfrm_fx) != 0 && sub(st_fx->last_extl_fx, WB_TBE) == 0 )
                    {
                ...
                ELSE
                {
                    ...
                    case 0:
                        ...
                        IF(L_sub(st_fx->GainFrame_prevfrm_fx, 327680l/*1.25 Q18*/) <= 0) temp = 26214/*0.8 Q15*/;
                        move16();
                        BREAK;
                    case 1:
                        GainFrame = 524288; /* 2.0f in Q18 */
                        IF(L_sub(st_fx->GainFrame_prevfrm_fx, 327680l/*1.25 Q18*/) > 0 && L_sub(st_fx->GainFrame_prevfrm_fx, 786432l/*3 Q18*/) <= 0) temp = 26214/*0.8 Q15*/;
                        move16();
                        test();
                        BREAK;
                    case 2:
                        GainFrame = 1048576;/* 4.0f in Q18 */
                        IF(L_sub(st_fx->GainFrame_prevfrm_fx, 786432l/*3 Q18*/) > 0 && L_sub(st_fx->GainFrame_prevfrm_fx, 1572864l/*6 Q18*/) <= 0) temp = 26214/*0.8 Q15*/;
                        move16();
                        test();
                        BREAK;
                    case 3:
                        GainFrame = 2097152;/* 8.0f in Q18 */
                        IF(L_sub(st_fx->GainFrame_prevfrm_fx, 1572864l/*6 Q18*/) > 0 && L_sub(st_fx->GainFrame_prevfrm_fx, 4194304l/*16 Q18*/) <= 0) temp = 26214/*0.8 Q15*/;
                        move16();
                        test();
                        BREAK;
                    default:
...
)
{
    ...
    test();
    IF( st_fx->bfi_fx && sub(st_fx->clas_dec,UNVOICED_CLAS) != 0 )
    {
        ...
        IF( !st_fx->bfi_fx )
        {
            IF(st_fx->use_partial_copy)
            {
                ...
                IF( sub(st_fx->rf_frame_type,RF_NELP) == 0 )
                {
                    ...
                    IF( sub(st_fx->core_fx,ACELP_CORE) == 0 && sub(st_fx->last_core_fx,ACELP_CORE) == 0
                        && !st_fx->prev_use_partial_copy && sub(st_fx->prev_coder_type_fx,UNVOICED) == 0
                        && L_sub(GainFrame,st_fx->GainFrame_prevfrm_fx) != 0 && sub(st_fx->next_coder_type,GENERIC) != 0 && sub(st_fx->last_extl_fx, SWB_TBE) == 0)
                    {
                        /*GainFrame = 0.2f*GainFrame + 0.8f*st_fx->GainFrame_prevfrm_fx;*/
                        GainFrame = L_add(Mult_32_16(st_fx->GainFrame_prevfrm_fx, 26214), Mult_32_16(GainFrame, 6553));
                    }
    ...
    test();
    IF( sub(coder_type, VOICED) == 0 || sub(mean_vf, 13107/*0.4f Q15*/ ) > 0 )
    {
        FOR( i = 1; i < NB_SUBFR; i++ )
        {
    ...
    IF( st_fx->bws_cnt_fx == 0 && st_fx->bws_cnt1_fx == 0 && st_fx->prev_use_partial_copy == 0 && st_fx->use_partial_copy == 0 )
    {
        ...
        IF( L_sub(st_fx->total_brate_fx,ACELP_16k40) <= 0 )
        {
               ...
               !(((sub(st_fx->prev_tilt_para_fx,3072) < 0 && sub(st_fx->prev_coder_type_fx,VOICED) >= 0)) && sub(tilt_para,5120) > 0))
            {
                FOR( i = 1; i < LPC_SHB_ORDER-1; i++ )
                {
                    IF(sub(lsf_diff[i],st_fx->prev_lsf_diff_fx[i-1]) < 0)
                    {
    ...
    IF( L_sub(st_fx->total_brate_fx, ACELP_32k) > 0 )
    {
        FOR( j = 0; j < 4; j++ )
        {
    ...
    *Q_white_exc = Q_bwe_exc_fb;
    ...
    if( sub( voice_factors[0], 24576/*0.75f Q15*/ ) > 0 )
    {
    ...
    IF( st_fx->bfi_fx )
    {
        ...
        ELSE IF (sub(st_fx->nbLostCmpt,1) > 0 )
        {
            ...
            ELSE IF( L_sub(curr_frame_pow,  L_shr(st_fx->prev_swb_bwe_frame_pow_fx,1)) < 0 &&
                     (L_sub(st_fx->enerLL_fx, L_shr( st_fx->prev_enerLL_fx,1)) > 0 || L_sub(st_fx->enerLH_fx, L_shr(st_fx->prev_enerLH_fx,1)) > 0) &&
                     (st_fx->prev_coder_type_fx == UNVOICED || st_fx->last_good_fx == UNVOICED_CLAS || sub(st_fx->tilt_swb_fec_fx , 10240) > 0) )
            {
                L_tmp = root_a_over_b_fx( st_fx->prev_swb_bwe_frame_pow_fx, curr_frame_pow_exp, curr_frame_pow, curr_frame_pow_exp, &exp );
...
)
{
    ...
    ELSE /* SWB TBE DEC */
    {
...

        {
            ...
            {
                /* safety check in case of bit errors */
                IF(sub(lsf_q[LPC_SHB_ORDER - 1 - i], MAX_LSF_FX) > 0)
                {
                    st_fx->BER_detect = 1;
...
)
{
    ...
    ELSE IF( L_sub( target_fs, 16000 ) == 0 )
    {
