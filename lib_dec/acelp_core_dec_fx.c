...
)
{
    ...
    test();
    test();
    if( st_fx->last_con_tcx && sub(st_fx->L_frameTCX_past, st_fx->L_frame_fx) != 0 && st_fx->last_core_fx != 0 )
    {
    ...
    IF( sub(st_fx->last_L_frame_fx,st_fx->L_frame_fx) != 0 )
    {
        ...
        {
            move16();
            IF( sub(st_fx->last_L_frame_fx,L_FRAME32k) == 0 )
            {
                /* (float)12800/(float)32000; */
                k = 13107;
            }
            ELSE IF( sub(st_fx->last_L_frame_fx,512) == 0 )
            {
    ...
    IF( sub(st_fx->bfi_pitch_frame_fx, st_fx->L_frame_fx) != 0 )
    {
        IF( sub(st_fx->L_frame_fx,L_FRAME) == 0 )
        {
            move16();
            IF( sub(st_fx->bfi_pitch_frame_fx,L_FRAME32k) == 0 )
            {
                /* (float)12800/(float)32000; */
                k = 13107;
            }
            ELSE IF( sub(st_fx->bfi_pitch_frame_fx,512) == 0 )
            {
                /* (float)12800/(float)25600; */
                k = 16384;
            }
    ...
    IF( !st_fx->bfi_fx )
    {
        ...
        ELSE
        {
            ...
            IF( L_sub(st_fx->core_brate_fx,ACELP_11k60) >= 0 )
            {
                ...
                test();
                test();
                IF( sub(st_fx->last_core_fx,HQ_CORE) != 0 || (sub(st_fx->last_core_fx,HQ_CORE) == 0 && st_fx->last_con_tcx) )
                {
                    test();
                    test();
                    test();
            ...
            *------------------------------------------------------------*/
            ...
            ELSE
            {
                ...
                test();
                test();
                if( sub(coder_type_fx, INACTIVE) == 0 && st_fx->flag_cna && sub(round_fx(L_shl(st_fx->lp_noise,1)), 15<<7) >= 0 )
                {
                    k = 1;
                    move16();
                }
            ...
            test();
            if( (L_sub(st_fx->total_brate_fx,ACELP_7k20) == 0) || (L_sub(st_fx->total_brate_fx,ACELP_8k00) == 0) )
            {
    ...
    test();
    test();
    IF( st_fx->flag_cna || (sub(st_fx->cng_type_fx,FD_CNG) == 0 && L_sub(st_fx->total_brate_fx,ACELP_32k) <= 0) || (sub(st_fx->cng_type_fx,LP_CNG) == 0 && L_sub(st_fx->total_brate_fx, SID_2k40) <= 0) )
    {
    ...
    IF( st_fx->flag_cna == 0 && sub(st_fx->L_frame_fx,L_FRAME16k) == 0 && st_fx->last_flag_cna == 1 && ( (st_fx->last_core_fx == ACELP_CORE && st_fx->last_coder_type_fx != AUDIO) || st_fx->last_core_fx == AMR_WB_CORE) )
    {
    ...
    *----------------------------------------------------------------*/
    ...
    test();
    IF( sub(st_fx->L_frame_fx,st_fx->last_L_frame_fx) != 0 && sub(st_fx->last_codec_mode,MODE2) != 0 )
    {
        IF( sub(st_fx->L_frame_fx,L_FRAME) == 0 )
        {
    ...
    test();
    test();
    IF( ( !st_fx->bfi_fx && ( st_fx->prev_bfi_fx )) || ((sub(st_fx->last_vbr_hw_BWE_disable_dec_fx,1) == 0) && (st_fx->vbr_hw_BWE_disable_dec_fx == 0)) || ((sub(st_fx->extl_fx,SWB_TBE)==0 || sub(st_fx->extl_fx,WB_TBE)==0 || sub(st_fx->extl_fx,FB_TBE)==0) && sub(st_fx->last_extl_fx,SWB_TBE)!=0 && sub(st_fx->last_extl_fx,WB_TBE)!=0 && sub(st_fx->last_extl_fx,FB_TBE)!=0) )
    {
