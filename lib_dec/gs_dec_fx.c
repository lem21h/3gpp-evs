...
)
{
    ...
    test();
    IF( st_fx->GSC_noisy_speech_fx && sub(st_fx->bwidth_fx,SWB) != 0 )
    {
        st_fx->BER_detect = 1;
        move16();
    ...
    IF( pit_band_idx != 0 )
    {
        ...
        IF( sub(pit_band_idx,13) > 0 ) /* The maximum decodable index is 10 + BAND1k2 (3) = 13 */
        {
            pit_band_idx = 13;
            move16();
            st_fx->BER_detect = 1;
...
)
{
    ...
    *last_bin = 0;
    ...
    test();
    IF( (sub(st_fx->bfi_fx,1) == 0) || st_fx->BER_detect )
    {
        /*--------------------------------------------------------------------------------------*
         * Copy old spectrum
    ...
    ELSE
    {
        ...
        test();
        IF( sub(st_fx->last_coder_type_fx, AUDIO) != 0 /* First audio frame */
        && sub(st_fx->last_coder_type_fx, UNVOICED) != 0 )/* last_coder_type == INACTIVE is overwritten in update_dec to UNVOICED */
        {
