...
)
{
    ...
    IF( !st_fx->bfi_fx )
    {
        ...
        IF ( L_sub(st_fx->core_brate_fx,FRAME_NO_DATA) == 0 || L_sub(st_fx->core_brate_fx,SID_1k75) == 0 )
        {
            ...
            move16();
            test();
            if( L_sub(st_fx->lp_ener_fx,40) < 0 ) /* very low energy frames, less than 0.3125 */
            {
                delta_mem_scale = 0;
                move16();
            }
        ...
        ELSE
        {
            ...
            test();
            test();
            if(sub(coder_type,INACTIVE) == 0 && st_fx->flag_cna && sub(st_fx->psf_lp_noise_fx,15<<7) >= 0)
            {
                tmp16 = 1;
                move16();
            }
    ...
    IF( (sub(st_fx->psf_lp_noise_fx,15<<8) >= 0) || (coder_type == INACTIVE) )
    {
        ...
        test();
        IF( sub(st_fx->VAD, 1) == 0 && sub(coder_type,INACTIVE) != 0 )
        {
            st_fx->VAD = 1;
            move16();
        }
        ...
        test();
        IF( st_fx->flag_cna && sub(st_fx->psf_lp_noise_fx,15<<8) >= 0 )
        {
            flag_cna = 1;
            move16();
    ...
    IF( flag_cna == 0 )
    {
        ...
        test();
        test();
        IF( sub(st_fx->last_flag_cna,1) == 0 && ( (sub(st_fx->last_core_fx,ACELP_CORE) == 0 && sub(st_fx->last_coder_type_fx,AUDIO) != 0) || sub(st_fx->last_core_fx,AMR_WB_CORE) == 0 ) )
        {
    ...
    test();
    test();
    IF( sub(output_frame,L_FRAME16k) >= 0 && (sub(st_fx->cldfbSyn_fx->bandsToZero, sub( st_fx->cldfbSyn_fx->no_channels, 10 )) != 0 || sub(st_fx->last_flag_filter_NB, 1) != 0 ) )
    {
