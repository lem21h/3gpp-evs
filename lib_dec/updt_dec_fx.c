...
)
{
    ...
    IF( sub(st_fx->last_core_fx,AMR_WB_CORE) == 0 )      /* switching to EVS primary mode */
    {
        ...
        move16();
        move16();
        if(sub(st_fx->last_flag_filter_NB, 1) == 0)
...
)
{
    ...
    test();
    test();
    IF( (sub(st_fx->rf_frame_type,RF_TCXFD) >= 0 && sub(st_fx->rf_frame_type,RF_TCXTD2) <= 0 && st_fx->use_partial_copy && st_fx->bfi_fx) || !st_fx->bfi_fx )
    {
        test();
        test();
        test();
        if( st_fx->bfi_fx && (sub(st_fx->last_good_fx, UNVOICED_TRANSITION) <= 0) && (sub(st_fx->clas_dec, UNVOICED_TRANSITION) > 0) && st_fx->last_con_tcx )
        {
