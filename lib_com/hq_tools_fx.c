...
)
{
    ...
    *noise_level = 0;
    ...
    move16();
    test();
    WHILE (L_sub(L_deposit_h(ener_limit), fact) < 0 && sub(add(i,1), nb_sfm) < 0)
    {
        ener_limit = add(ener_limit, wnorm[++i]);
        test();
    }
...
)
{
    ...
    IF( is_transient == 0 )
    {
        ...
        test();
        IF(sub(last_sfm,8) > 0 && total_bit > 0)
        {
        ...
        FOR( sfm = 0; sfm < num_sfm; sfm++ )
        {
            ...
            IF(R[sfm] != 0 && sub(R[sfm], shl(mult(24756,sfmsize[sfm]),1)) < 0)
            {
                ...
                IF (env_fx > 0 )
                {
                    ...
                    test();
                    test();
                    IF((sub(bitalloc_var_fx,4915) > 0 || L_sub(L_normq_v[sfm], peak_fx)<0) && peak_fx != 0)
                    {
                        Word16 exp_p;
               ...
               && L_sub(prev_normq_fx[sfm],L_shl(L_normq_v[sfm],1)) < 0 && sub(bitalloc_var_fx,4915) <= 0)
            {
                ...
                FOR (i = add(sfm_start[sfm],12); i < sfm_end[sfm]; i++)
                {
                    ...
                    test();
                    test();
                    IF(L_sub(L_abs(L_coeff_out[i]),L_shl(L_abs(*p_prev_coeff_out),2)) >0
                       || L_sub(L_abs(L_coeff_out[i]),L_shr(L_abs(*p_prev_coeff_out),2)) <0
