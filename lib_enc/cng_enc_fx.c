...
)
{
    ...
    ELSE
    {
        ...
        test();
        test();
        IF( L_sub(st_fx->last_core_brate_fx, SID_2k40) > 0 && ( sub( st_fx->last_core_fx, HQ_CORE )==0  || burst_ho_cnt > 0 ) && sub(st_fx->lp_sp_enr_fx, 1536) < 0 &&
    ...
    *allow_cn_step = 0;
    ...
    IF( ((st_fx->cng_cnt_fx == 0) &&
         ...
         (sub(add(st_lp_sp_enr, 1024 /* 4.0, Q8 */), sp_enr) < 0) &&
