...
)
{
    ...
    test();
    IF( sub(coder_type_fx, VOICED) == 0 || sub(mean_vf, 13107/*0.4f Q15*/ ) > 0 )
    {
        FOR( i = 1; i < NB_SUBFR; i++ )
        {
    ...
    *Q_white_exc = Q_bwe_exc_fb;
    ...
    if( sub( voice_factors_fx[0], 24576/*0.75f Q15*/ ) > 0 )
    {
    ...
    IF( sub(st_fx->cldfbHBLT, 8192/*1.0f Q13*/) < 0 )   /* cldfbHBLT in Q13 */
    {
    ...
    test();
    IF( sub(frGainAttenuate, 1) == 0 && sub( MA_lsp_shb_spacing, 79/*0.0024f Q15*/ ) <= 0 )
    {
    ...
    *MA_lsp_shb_spacing = mac_r(tempQ31, lsp_spacing, 13107);      /* 0.4f */
        ...
        || sub( lsp_spacing, 105 ) <= 0 )
    {
        *frGainAttenuate = 1;
        ...
        test();
        IF ( L_sub( lsp_slow_evol_rate, 2147484l/*0.001f Q31*/ ) < 0 &&  L_sub( lsp_fast_evol_rate, 2147484l/*0.001f Q31*/ ) < 0 )
        {
            *frGainSmoothEn = 1;
            move16();
        }
...
)
{
    ...
    IF(sub(idxGain,2) > 0 && sub(idxGain,15) < 0 && sub(ratio,add(shl(2,sub(idxGain,1)),shl(2,sub(idxGain,2)))) > 0)
    {
