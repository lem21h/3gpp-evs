...
)
{
    ...
    *-------------------------------------------------------------------------------------*/
    ...
    ELSE IF((sub(stab, STAB_FAC_LIMIT_FX) < 0) && (sub(st_fx->clas_fx, VOICED_CLAS)==0 || (sub(st_fx->clas_fx,VOICED_CLAS) < 0 && sub(coder_type,AUDIO) == 0) ) )
    {
        st_fx->next_force_safety_net_fx = 1;
        move16();
    }
...
)
{
    ...
    /* quantize first stage with 4 bits
       ...
       the decoded frame is WB2 with sampling rate of 16 kHz, otherwise it is sampled at 12.8kHz and contains
       either NB or WB LSF data.  */
    IF(sub(lsf[M - 1], WB_LIMIT_LSF_FX) > 0) /* 16kHz sampled LSF vector*/
    {
        p_cb = &CNG_SN1_fx[0];
        move16();
        first_cb = 0;
