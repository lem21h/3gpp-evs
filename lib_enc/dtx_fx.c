...
)
{
    ...
    IF( st_fx->Opt_DTX_ON_fx && vad == 0 &&
        ...
        sub(st_fx->last_core_fx, AMR_WB_CORE) != 0 &&
        st_fx->Opt_AMR_WB_fx == 0 )
    {
        ...
        IF ( L_sub(st_fx->last_core_brate_fx,SID_2k40) > 0 &&
             L_sub(st_fx->last_total_brate_cng_fx,-1) != 0 &&
             L_sub(st_fx->last_total_brate_cng_fx,st_fx->total_brate_fx) != 0 &&
             ( (L_sub(st_fx->last_total_brate_cng_fx, ACELP_24k40) <= 0) || (sub(st_fx->lp_noise_fx, 1280) < 0) ) )
        {
            st_fx->total_brate_fx = st_fx->last_total_brate_cng_fx;
            move32();
            test();
            if( !(L_sub(st_fx->total_brate_fx,ACELP_7k20) == 0 && st_fx->Opt_SC_VBR_fx) )
            {
        ...
        test();
        IF ( L_sub(st_fx->last_core_brate_fx,SID_2k40) <= 0 &&
             L_sub(st_fx->last_total_brate_fx,st_fx->total_brate_fx) != 0 &&
             ( L_sub(st_fx->last_total_brate_fx,ACELP_24k40) <= 0 || sub(st_fx->lp_noise_fx, 1280) < 0) )

        {
            ...
            test();
            test();
            if( st_fx->rf_mode && st_fx->rf_fec_offset > 0 && L_sub(st_fx->total_brate_fx,ACELP_13k20) == 0 && sub(st_fx->bwidth_fx,NB) != 0 )
            {
                st_fx->Opt_RF_ON = 1;
                move16();
            }
    ...
    IF ( (L_sub(st_fx->core_brate_fx ,SID_2k40) != 0 ) && (L_sub(st_fx->core_brate_fx, SID_1k75) != 0 ) && (st_fx->core_brate_fx != 0))
    {
        ...
        IF( sub(st_fx->active_fr_cnt_fx,CNG_TYPE_HO) >= 0 && st_fx->Opt_AMR_WB_fx == 0 )
        {
            ...
            test();
            test();
            IF( sub(st_fx->cng_type_fx,LP_CNG) == 0 && ( (sub(st_fx->input_bwidth_fx,NB) == 0 && L_sub(st_fx->bckr_tilt_lt,589824l/*9.f Q16*/) > 0) || (sub(st_fx->input_bwidth_fx,NB) > 0 && L_sub(st_fx->bckr_tilt_lt,2949120l/*45.f Q16*/) > 0) ) )
            {
                st_fx->cng_type_fx = FD_CNG;
                move16();
            }
            ELSE IF( sub(st_fx->cng_type_fx,FD_CNG) == 0 && ( (sub(st_fx->input_bwidth_fx,NB) == 0 && L_sub(st_fx->bckr_tilt_lt,131072l/*2.f Q16*/) < 0) || (sub(st_fx->input_bwidth_fx,NB) > 0 && L_sub(st_fx->bckr_tilt_lt,655360l/*10.f Q16*/) < 0) ) )
            {
                st_fx->cng_type_fx = LP_CNG;
                move16();
            }
    ...
    IF(  st_fx->Opt_DTX_ON_fx && (st_fx->core_brate_fx == SID_2k40 || st_fx->core_brate_fx == FRAME_NO_DATA ) )
    {
        ...
        move16();
        test();
        if( L_sub(st_fx->last_core_brate_fx, SID_2k40) > 0 && L_sub(st_fx->last_total_brate_cng_fx, -1) != 0 )
        {
            st_fx->bwidth_fx = st_fx->last_bwidth_cng_fx;
            move16();
        }
        ...
        test();
        test();
        IF( st_fx->Opt_RF_ON && L_sub(st_fx->total_brate_fx, ACELP_13k20) != 0 )
        {
            reset_rf_indices(st_fx);
            move16();
            st_fx->Opt_RF_ON = 0;
