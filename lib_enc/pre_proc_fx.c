...
)
{
    ...
    *attack_flag = 0;
    ...
    if( ((sub(st->bwidth_fx, NB) == 0) || (sub(st->max_bwidth_fx, NB) == 0)) && (L_sub(st->input_Fs_fx, 8000) > 0) )
    {
    ...
    *vad_flag = wb_vad_fx( st, fr_bands, localVAD, &noisy_speech_HO, &clean_speech_HO, &NB_speech_HO,
    ...
    IF ( sub( st->Pos_relE_cnt, 20) < 0)          /* Ensure the level is high enough and cldfb decision is reliable */
    {
    ...
    IF( (L_sub(st->last_total_brate_fx,st->total_brate_fx) != 0) || (sub(st->last_bwidth_fx,st->bwidth_fx) != 0) )
    {
        ...
        move32();
        test();
        if( st->rf_mode && L_sub(st->total_brate_fx,ACELP_13k20) == 0 )
        {
            L_tmp = ACELP_9k60;
            move32();
        }
    ...
    *-----------------------------------------------------------------*/
    ...
    IF( sub(st->codec_mode,MODE1) == 0 )
    {
        ...
        ELSE IF( L_sub(st->total_brate_fx,ACELP_24k40) > 0 &&
                 ( (*vad_flag == 0 && sub(st->bwidth_fx,SWB) >= 0 && sub(st->max_bwidth_fx,SWB) >= 0) || (*localVAD == 0 && (sub(st->bwidth_fx,WB) <= 0 || sub(st->max_bwidth_fx,WB) <= 0)) )
               )
        {
