...
)
{
    ...
    ELSE
    {
        ...
        *Etot_l = s_min(*Etot_l, Etot);
        ...
        IF ( sub(ini_frame_fx,100)<0 && sub(*Etot_l,*Etot_l_lp)<0 )
        {
...
)
{
    ...
    FOR( i = st_fx->min_band_fx; i <= st_fx->max_band_fx; i++ )
    {
        ...
        IF ( sub(i,START_BAND_SPMUS) >= 0 && sub(i,NB_BANDS_SPMUS+START_BAND_SPMUS) < 0 )
        {
            ...
            *non_staX                 = add(*non_staX, wtmp);
            ...
            IF ( sub(st_fx->ini_frame_fx, 100 ) >= 0 )
            {
    ...
    *st_harm_cor_cnt = add(*st_harm_cor_cnt , 1);
    ...
    IF( (sub(*st_harm_cor_cnt,1) > 0 ) &&( ( sub(Etot,3840) < 0 ) ||   /* 15 in Q8 */
                                           ( sub(st_fx->ini_frame_fx,10) > 0  &&
                                                   sub(sub(Etot,st_fx->Etot_lp_fx),1792)>0 )) /* 7 in Q8 */
      )
    {
        *st_harm_cor_cnt = 1;
    }

    if ( sub(*st_harm_cor_cnt,1) > 0 &&
            sub(Etot,7680) > 0 &&             /* 30.0f in Q8 */
            sub(st_E_var_est_fx,32 )>0        /* 8.0f in Q2 */
       )
    {
    ...
    if (  ( (sub(st_fx->act_pred_fx, 19333) < 0 ) && ( sub(st_fx->lt_haco_ev_fx, 7537) < 0 ) ) /* .59 in Q15 .23 in Q15 */
            ...
            || aE_bgd != 0
            || ( (sub(Etot,10752)<0)                         /* 42 in Q8 */
                 && (sub(st_fx->harm_cor_cnt_fx, 10) > 0 )
                 && ( sub(st_fx->lt_haco_ev_fx, 11469) < 0 ) /* 0.35 in Q15 */
