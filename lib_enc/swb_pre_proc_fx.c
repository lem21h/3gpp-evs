...
)
{
    ...
    test();
    test();
    IF ( (L_sub(st_fx->last_total_brate_fx, ACELP_6k60)==0) ||
         (L_sub(st_fx->last_total_brate_fx, ACELP_8k85)==0) ||
         (L_sub(st_fx->last_total_brate_fx, ACELP_12k65)==0) ||
         (L_sub(st_fx->last_total_brate_fx, ACELP_14k25)==0) ||
         (L_sub(st_fx->last_total_brate_fx, ACELP_15k85)==0) ||
         (L_sub(st_fx->last_total_brate_fx, ACELP_18k25)>=0 && L_sub(st_fx->last_total_brate_fx, ACELP_23k85)<=0) )
    {
        fSwitchFromIO = 1;
        move16();
    }
...
)
{
    ...
    IF( L_sub(st_fx->input_Fs_fx, 32000) == 0 )
    {
        ...
        test();
        test();
        IF( sub(st_fx->last_extl_fx, SWB_BWE) != 0 && sub(st_fx->last_extl_fx, FB_BWE) != 0 && sub(st_fx->extl_fx, SWB_BWE_HIGHRATE) != 0 )
        {
